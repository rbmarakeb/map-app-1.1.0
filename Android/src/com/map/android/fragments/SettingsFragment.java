...
import com.map.android.activities.helpers.MapPreferencesActivity;
...
import com.map.android.fragments.widget.WidgetsListPrefFragment;
...
public class SettingsFragment extends PreferenceFragment implements OnSharedPreferenceChangeListener,
     ...
     * Action used to broadcast updates to the gps hdop display preference.
     */
    public static final String ACTION_PREF_JOYSTICKCHANNEL_UPDATE = PACKAGE_NAME + ".ACTION_PREF_JOYSTICKCHANNEL_UPDATE";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setupMapPreferences();
        setupAltitudePreferences();
        //setupJoystickChannelPreferences();
    }
    ...
    private void setupJoystickChannelPreferences(){
        ListPreference joystickChannelPref = (ListPreference) findPreference(DroidPlannerPrefs.PREF_JOYSTIC_CHANNEL);
        if(joystickChannelPref != null){
            String defaultChannel = dpPrefs.getJoystickChannel();
            joystickChannelPref.setSummary(defaultChannel);
            updateJoystickChannelSummary(joystickChannelPref, defaultChannel);
            joystickChannelPref.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    String channel = (String) newValue;
                    updateJoystickChannelSummary(preference, channel);
                    lbm.sendBroadcast(new Intent(ACTION_PREF_JOYSTICKCHANNEL_UPDATE));
                    return true;
                }
            });
        }
    ...
    private void updateJoystickChannelSummary(Preference preference, String channel){
        preference.setSummary(channel);
    }
    ...
    private void updateFirmwareVersionPreference(Type droneType) {
        ...
        if (firmwareVersionPref != null) {
            ...
            if (firmwareVersion == null) {
                firmwareVersionPref.setSummary("Not Connected");
                firmwareEvent.setAction("Firmware version unset");
            } else {
                firmwareVersionPref.setSummary("MAP160724");
