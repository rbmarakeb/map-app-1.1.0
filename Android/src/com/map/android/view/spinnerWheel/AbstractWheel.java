...
package com.map.android.view.spinnerWheel;

import java.util.LinkedList;
import java.util.List;

import com.map.android.R;
import com.map.android.view.spinnerWheel.adapters.WheelViewAdapter;
...
public abstract class AbstractWheel extends View {
	...
	protected void initAttributes(AttributeSet attrs, int defStyle) {
		TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.AbstractWheelView,
				defStyle, 0);
		...
		mScroller = createScroller(new WheelScroller.ScrollingListener() {
			...
			public void onTouchUp() {
				if (!mIsScrollingPerformed)
					onScrollTouchedUp(); // if scrolling IS performed, whe
	...
	public void scroll(int itemsToScroll, int time) {
		...
		onScrollTouched(); // we have to emulate touch when scrolling
							// spinnerwheel programmatically to light up stuff
	...
	/**
	 * Returns base dimension of the spinnerwheel — width for horizontal
	 * spinnerwheel, height for vertical
	 * 
	...
	/**
	 * Returns base dimension of base item — width for horizontal spinnerwheel,
	 * height for vertical
	 * 
	 ...
	 * depends on spinnerwheel layout parameters. To apply changes and rebuild
	 * view call measure().
	 * 
	 * @param count
	...
	/**
	 * Tests if spinnerwheel is cyclic. That means before the 1st item there is
	 * shown the last one
	 * 
	...
	protected void notifyClickListenersAboutClick(int item, boolean isCurrent) {
		for (OnWheelClickedListener listener : clickingListeners) {
			listener.onItemClicked(this, item, isCurrent);
		}
	}
	 ...
	 * @param index
	 *            the item index
	 * @return true if item index is not out of bounds or the spinnerwheel is
	 *         cyclic
	 */
		...
		case MotionEvent.ACTION_UP:
			if (!mIsScrollingPerformed) {
				...
				if (isValidItemIndex(mCurrentItemIdx + items)) {
					notifyClickListenersAboutClick(mCurrentItemIdx + items, items == 0);
				}
			}
			break;
		}
		return mScroller.onTouchEvent(getParent(), event);
	}
